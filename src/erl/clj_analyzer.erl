...
ensure_specs_loaded() ->
  case clj_cache:get(?SPECS_LOADED) of
    ?NIL ->
...
-spec should_check_specs() -> boolean().
should_check_specs() ->
  case erlang:get(?CHECK_SPECS) of
    ?NIL ->
      Result = clj_cache:get(?CHECK_SPECS) == {ok, true},
      erlang:put(?CHECK_SPECS, Result),
      Result;
    Result ->
      Result
  end.
